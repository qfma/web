...
- DUF1120 domain counts in CDS depend on correct alignment and merging of all CDS sequences, slight variations possible
...
### DUF1120 in merged CDS

The first two analysis, in peptide and cDNA sequences, always used the longest isoform for a given gene and therefore may not capture all DUF1120 domains present, because some shorter, non-overlapping isoforms could contain DUF1120 domains. These domains would then not be counted using only the longest isoform. Exon sequences are unfortunately isoform depended in the Ensembl database and cannot be easily obtained. For this reason we obtained all coding sequences for every gene that was shown to contain DUF1120 domains using the cDNA analysis. We then aligned all CDS using ```prank``` with a higher gaprate (0.8) and a lower gaprate (0.01). The alignment was then used to build a consensus sequence with EMBOSS ```cons```, which should merge non-overlapping isoforms and thus count all DUF1120 domains in the genome.

```bash
./align-cds-consensus.sh

# Prank options used
prank -d=$i -o=$f.cds -gapext=0.8 -gaprate=0.01
# EMBOSS cons options used
cons -identity 0 -plurality 1
```
...
We used this information to compile a fourth DUF1120 domain count, including only genes that have a valid CM promoter.
